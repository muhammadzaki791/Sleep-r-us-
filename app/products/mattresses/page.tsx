...
export default function MattressesPage() {
  ...
  useEffect(() => {
    const fetchCategoryData = async () => {
      try {
        const data = await client.fetch(getCategoryBySlugWithProductsQuery, { slug: 'mattresses' })
        setCategoryData(data)
      } catch (error) {
        console.error('Error fetching mattresses category and products:', error)
      } finally {
        setLoading(false)
      }
    }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="mt-4 text-[#0B3D91]">Loading mattresses...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white py-16">
        <div className="container mx-auto px-4">
          <FadeIn>
            <h1 className="text-4xl font-bold text-center text-[#0B3D91] mb-6">Premium Mattresses</h1>
            <p className="text-xl text-gray-600 text-center mb-12">Discover the perfect night's rest</p>
          </FadeIn>

          <div className="text-center py-12">
            <p className="text-gray-600 text-lg">Mattress category not found or no products available at the moment. Please check back later.</p>
          </div>
        </div>

        <WhatsAppButton message="Hello, I'm interested in your mattresses. Can you provide more information?" />
      </div>
    )
  }
      ...
      <div className="container mx-auto px-4">
        ...
        {categoryData.directProducts && categoryData.directProducts.length > 0 && (
          <StaggerContainer>
            <div className="mb-16">
              <FadeIn>
                <h2 className="text-2xl font-bold text-center text-[#0B3D91] mb-8">Premium Mattresses</h2>
              </FadeIn>
        ...
        {(!categoryData.directProducts || categoryData.directProducts.length === 0) &&
         (!categoryData.subCategories || categoryData.subCategories.length === 0) && (
          <div className="text-center py-12">
            <p className="text-gray-600 text-lg">No mattresses available at the moment. Please check back later.</p>
          </div>
        )}
      </div>
